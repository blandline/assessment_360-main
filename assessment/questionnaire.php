...
if (isset($_GET["id"]) && isset($_GET["password"])) {
  $rater_id = $_GET["id"];
  $password = Mcrpty::_decrypt($_GET["password"]);
  $dbName = substr($password, 4, -4);
...
<?
if(!isset($questionnaireviewincluded)){
  $questionnaireviewincluded = true;
  include_once("../views/member/assess360questionnaireView.php");
}
